...
private:
	...
	bool directed = true;
    std::unordered_map<int, std::set<Ref, Compare<T, W> > > clusters;
...
public:
	Graph(T n, T m, bool _directed = true) : vertexes(n), directed(_directed)
	{
		for (T i = 0; i < n; i++) {
            ...
            std::set<Ref, Compare<T, W>> to_add;
            to_add.insert(vertexes[i]);
            clusters[i] = to_add;
		}
	...
	void find_communities() {
		...
		while (!stop) {
            stop = true;
			for (auto vertex : vertexes) {
                clusters[vertex->get_num()].erase(vertex);
                feedback = vertex->step(directed, vertexes);
                if (feedback) {
                    stop = false;
                    for (auto v : clusters[vertex->get_num()]) {
                        if (v->get_cluster() == vertex->get_cluster()) {
                            v->set_profit(v->compute_profit(vertex->get_cluster()));
                        }
                    }
                }
                clusters[vertex->get_num()].insert(vertex);
            }
		}
	}
